...
We are building a Student attendance tracking mobile app using Expo.

The Student Attendance Tracker is a personal attendance management application designed to help college students stay aware of their attendance throughout the semester. Instead of waiting for professors to announce attendance percentages or manually calculating them, the app keeps all attendance-related information in one place. It acts like a personal academic companion that helps students understand their current attendance status, stay prepared for upcoming classes, and avoid falling below the required attendance percentage.

Many students do not know how many classes they have attended, how many they have missed, or how many more they can safely skip while still meeting the college attendance requirement. This often leads to unnecessary stress near the end of the semester.
The purpose of this application is to remove that uncertainty by automatically organizing attendance records and presenting them in a simple and meaningful way. It helps students make informed decisions about their attendance every day instead of relying on rough estimates or manual calculations.

The journey begins when a student creates a new semester by entering basic academic information such as the semester duration, subjects, weekly timetable, holidays, and the minimum attendance requirement set by the college.
Once the semester is ready, the app follows the student's daily schedule. Every day, it shows the classes planned for that day and allows the student to mark whether they attended or missed each lecture. As attendance is recorded, every part of the application updates automatically.
The dashboard gives an instant overview of the student's academic progress, while individual subject pages display detailed attendance information for each course. The timetable always reflects the current semester schedule, and the analytics section transforms attendance records into meaningful insights that help students understand their progress over time.
As the semester continues, the app becomes a complete history of the student's attendance, making it easy to monitor performance without maintaining spreadsheets or doing manual calculations.
...
- selectedClass: The currently viewed class or subject.
- attendanceData: The local state for attendance entries before syncing with the Spring Boot backend.
- userSettings: Preferences like theme (light/dark mode) or notification settings for attendance reminders.
- sessionState: Information about the current user session (e.g., student or teacher role).
- filterCriteria: The current date or subject filter being applied to the student list.
  Use AsyncStorage persistence for:
- attendanceData: To ensure progress is saved locally if the user is offline or loses connection before submitting to the backend.
- userSettings: To remember user preferences between app launches.
- authTokens: To securely persist login sessions so users don't have to re-authenticate every time they open the app.
