...
- Set the framework-specific client key (for example, `VITE_CLERK_PUBLISHABLE_KEY` for Vite or `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` for Next.js) and keep the server key as `CLERK_SECRET_KEY`
...
2. **Create migration plan** - Consider:
   ...
   - **OAuth identity continuity** - Map every legacy Google/GitHub provider and provider identifier to the corresponding Clerk external account before cutover. Validate imported users retain the correct linked identities without duplicate users or duplicate provider links. If mapping or validation fails, stop the cutover, restore the prior provider/session path, remove or quarantine the affected Clerk imports, correct the mapping, and re-run validation before retrying.
