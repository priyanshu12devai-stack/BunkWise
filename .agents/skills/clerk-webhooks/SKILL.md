...
export async function POST(req: NextRequest) {
  ...
  if (!webhookId) return new Response('Missing svix-id', { status: 400 })

  const result = await db.$transaction(async (tx) => {
    try {
      // webhookEvents.id must be unique. The insert and domain writes commit together.
      await tx.webhookEvents.create({ data: { id: webhookId, type: evt.type } })
    } catch (error) {
      // Only a duplicate webhookEvents.id is treated as an already-processed delivery.
      const target =
        error instanceof Prisma.PrismaClientKnownRequestError
          ? error.meta?.target
          : undefined
      const duplicatesWebhookId =
        error instanceof Prisma.PrismaClientKnownRequestError &&
        error.code === 'P2002' &&
        (target === 'id' || (Array.isArray(target) && target.includes('id')))
      if (duplicatesWebhookId) {
    ...
    if (evt.type === 'user.created' || evt.type === 'user.updated') {
      ...
      await tx.users.upsert({ where: { clerkId: id }, create: { clerkId: id, ...data }, update: data })
    }

    if (evt.type === 'user.deleted') {
      const { id } = evt.data
      await tx.users.deleteMany({ where: { clerkId: id } })
    }
    ...
    if (evt.type === 'organizationMembership.deleted') {
      ...
      await tx.teamMembers.deleteMany({
        where: { orgId: organization.id, userId: public_user_data.user_id },
      })
    }
...
export async function POST(req: NextRequest) {
  ...
  // Step 2: Persist a unique, durable notification job before acknowledging.
  ...
  // Acknowledge before a worker performs non-idempotent network calls.
  return new Response('Accepted', { status: 202 })
}

// The NotificationJob model stores claimToken, claimExpiresAt, completedAt,
// emailStatus/emailError, and slackStatus/slackError alongside the fields above.
// Run this from a durable queue worker with bounded retries.
export async function processUserCreatedNotification(job: NotificationJob) {
  const claimToken = crypto.randomUUID()
  const now = new Date()
  const claimExpiresAt = new Date(now.getTime() + 5 * 60_000)
  const claimed = await db.notificationJobs.updateMany({
    where: {
  ...
  const persistedJob = await db.notificationJobs.findUniqueOrThrow({
    where: { webhookId: job.webhookId },
  })
  if (persistedJob.claimToken !== claimToken) return

  const name = `${persistedJob.firstName ?? ''} ${persistedJob.lastName ?? ''}`.trim()
  const safeName = escapeHtml(name)
  const failures: unknown[] = []

  if (persistedJob.email && persistedJob.emailStatus !== 'succeeded') {
    try {
      const { error } = await resend.emails.send({
        from: 'noreply@yourdomain.com',
        to: persistedJob.email,
        subject: 'Welcome!',
        html: `<p>Hi ${safeName}, welcome to our app!</p>`,
      }, { idempotencyKey: `${job.webhookId}-welcome-email` })
      if (error) throw error
      await db.notificationJobs.updateMany({
        where: { webhookId: job.webhookId, claimToken },
        data: { emailStatus: 'succeeded', emailError: null },
      })
    } catch (error) {
      failures.push(error)
      await db.notificationJobs.updateMany({
        where: { webhookId: job.webhookId, claimToken },
        data: { emailStatus: 'failed', emailError: String(error) },
      })
    }
  }

  if (persistedJob.slackStatus !== 'succeeded') {
    try {
      const response = await fetch(process.env.SLACK_WEBHOOK_URL!, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: `New user signed up: ${name} (${persistedJob.email ?? 'no email'})` }),
        signal: AbortSignal.timeout(5_000),
      })
      if (!response.ok) throw new Error(`Slack returned HTTP ${response.status}`)
      await db.notificationJobs.updateMany({
        where: { webhookId: job.webhookId, claimToken },
        data: { slackStatus: 'succeeded', slackError: null },
      })
    } catch (error) {
      failures.push(error)
      await db.notificationJobs.updateMany({
        where: { webhookId: job.webhookId, claimToken },
        data: { slackStatus: 'failed', slackError: String(error) },
      })
    }
  }

  const latest = await db.notificationJobs.findUniqueOrThrow({
    where: { webhookId: job.webhookId },
  })
  const complete =
    (latest.emailStatus === 'succeeded' || latest.emailStatus === 'skipped') &&
    latest.slackStatus === 'succeeded'
  await db.notificationJobs.updateMany({
    where: { webhookId: job.webhookId, claimToken },
    data: {
      claimToken: null,
  ...
  if (failures.length > 0) {
    throw new AggregateError(failures, `Notification job ${job.webhookId} is incomplete`)
  }
}
```
