...
> This skill targets clerk `2.2.0`. If `clerk --version` disagrees with this reviewed version, refresh the skill source through the repository's normal update path. The binary is always the source of truth, so run `clerk <command> --help` to verify anything this skill claims.
...
| ------------------------- | -------------------------------- |
| bun (`bun.lock*`)         | `bunx clerk@2.2.0`     |
| npm (`package-lock.json`) | `npx -y clerk@2.2.0`   |
| pnpm (`pnpm-lock.yaml`)   | `pnpm dlx clerk@2.2.0` |
| yarn >= 2 (`yarn.lock`)   | `yarn dlx clerk@2.2.0` |
...
The published npm package is **`clerk`**, not `@clerk/cli`. Never teach `npm install -g clerk` as the primary path. If the global CLI is stale or behaves differently from this skill, either install the reviewed version or fall back to the pinned runner form above.
