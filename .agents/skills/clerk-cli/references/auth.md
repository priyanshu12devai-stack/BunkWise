...
In agent mode, if already authenticated, it's a no-op. If no session is stored, the actual host flow opens a browser and waits for the localhost callback; it is not an unattended login.
In a sandbox, browser launch, localhost binding, or even the "already authenticated" check can fail if the
keychain or fallback credential file is blocked, so rerun on the host before
trusting a sandboxed auth failure. Those sandbox limitations do not describe the host login flow.
