...
export default function SignUpPage() {
  ...
  if (
    ...
    // For email OTP: wait until the email address is the only unverified requirement
    ...
    return (
      <>
        ...
        <form action={handleVerify}>
          ...
          {errors?.fields?.code?.message && <p>{errors?.fields?.code?.message}</p>}
    ...
    <>
      ...
      <form action={handleSubmit}>
        ...
        <div>
          ...
          {errors?.fields?.phoneNumber?.message && <p>{errors?.fields?.phoneNumber?.message}</p>}
        </div>
        <button type="submit" disabled={fetchStatus === 'fetching'}>
          Continue
        </button>
      </form>
      {errors?.global?.map((error) => <p key={error.message}>{error.message}</p>)}
