  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.error('Failed to register Expo push token; retry on the next session change.', error)
      }
    })()
  }, [isLoaded, user])
...
Prerequisites: `npx --no-install expo install expo-local-authentication` (plus `expo-secure-store` from setup), iOS `NSFaceIDUsageDescription` in `app.json` → `ios.infoPlist`.
