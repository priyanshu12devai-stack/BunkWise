...
```bash
npx --no-install expo install @clerk/expo expo-secure-store
# add expo-dev-client if the app will use native components or native sign-in hooks
npx --no-install expo install expo-dev-client
```

`npx --no-install expo install` (not `npm install`) uses the project-local Expo CLI and ensures Expo-SDK-compatible versions. Additional peer deps are per-strategy — install only what the selected flow needs:

| Feature | Install |
|---------|---------|
| Browser SSO/OAuth (`useSSO`) | `npx --no-install expo install expo-auth-session expo-web-browser` |
| Native Google sign-in | `npx --no-install expo install expo-crypto` |
| Native Apple sign-in | `npx --no-install expo install expo-apple-authentication` |
| Biometrics (`useLocalCredentials`) | `npx --no-install expo install expo-local-authentication` |
| Passkeys | `npx --no-install expo install @clerk/expo-passkeys` |

Alternative bootstrap: after showing the exact command and receiving explicit confirmation, `npx -y clerk@2.2.0 init --framework expo` installs `@clerk/expo` and writes the publishable key to the env file (it does not scaffold screens).
...
Verify `app.json` / `app.config.js` includes both plugins (`npx --no-install expo install` usually adds them):
...
The `@clerk/expo` plugin registers the native modules (iOS min deployment target 17.0), wires Google sign-in when configured, and optionally accepts a `theme` option for native components (see prebuilt-components.md). Plugin changes require a fresh prebuild: `npx --no-install expo prebuild --clean` or rerunning `npx --no-install expo run:*`.
...
Dev build commands: `npx --no-install expo run:ios` / `npx --no-install expo run:android` (or EAS Build). State this requirement before implementing anything in the "No under Expo Go" rows.
