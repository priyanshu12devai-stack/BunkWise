...
5. Verify by building, not just by writing:
   - Run the project's typecheck (`npx --no-install tsc --noEmit` or equivalent) so verification cannot download an undeclared tool.
   - Build and launch with the project-local CLI: `npx --no-install expo run:ios` / `run:android` for native features, `npx --no-install expo start` for Expo Go flows. If the build fails, fix and rebuild iteratively — build errors against the installed SDK are the ground truth when this skill and the SDK disagree. After ~5 failed fix attempts, stop and ask the developer how to proceed instead of thrashing.
...
1. **Publishable key** — Read from `process.env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` (`.env` file). Never `NEXT_PUBLIC_`, never hardcoded. If no key exists, ask the developer for one. The mutating bootstrap alternative, `npx -y clerk@2.2.0 init --framework expo`, installs the SDK and writes the env file; show the exact command and obtain explicit confirmation before running it.
...
3. **Factor availability** — Before implementing a specific strategy (SMS, email code, social provider), confirm it's enabled for the instance. If deriving the Frontend API host from the publishable key, validate it before WebFetch: require HTTPS; allow only explicitly trusted Clerk hosts (`clerk.com`, `clerk.dev`, `clerk.accounts.dev`, or their subdomains); and reject localhost plus private, loopback, link-local, or otherwise non-public IP targets. If the decoded host is not explicitly trusted, do not fetch it—default to developer confirmation in the dashboard (**User & authentication**). SMS in particular is instance-configuration-dependent — code written for a disabled factor fails at runtime, not build time.
