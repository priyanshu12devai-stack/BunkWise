...
For the operations below, skip spec fetching and prepare the exact command using these templates. Read-only GET operations may be executed directly. For every write operation, including each organization-creation and invitation step, show the prepared command, perform all mandatory secret and scope checks above, and obtain explicit user confirmation before execution.
...
limit=100
offset=0
created_at_after="$(($(date -d '7 days ago' +%s 2>/dev/null || date -v-7d +%s) * 1000))"

while :; do
  response="$(curl -fsS --get "https://api.clerk.com/v1/users" \
    -H "Authorization: Bearer $CLERK_SECRET_KEY" \
    --data-urlencode "limit=$limit" \
    --data-urlencode "offset=$offset" \
    --data-urlencode "order_by=-created_at" \
    --data-urlencode "created_at_after=$created_at_after")"

  printf '%s' "$response" | jq -r '
    "Found \(.data | length) users on this page (\(.totalCount) total):",
    (.data[] | "  \(.id): \(.email_addresses[0].email_address // \"no email\")")
  '
  page_count="$(printf '%s' "$response" | jq -er '.data | length')"
  total_count="$(printf '%s' "$response" | jq -er '.totalCount')"
  offset=$((offset + page_count))
  [ "$offset" -lt "$total_count" ] || break
done
```
...
```
GET /v1/users
Query params: limit (max 500, default 10), offset, order_by (+/-created_at, +/-updated_at, +/-email_address, +/-web3wallet, +/-first_name, +/-last_name, +/-phone_number, +/-username, +/-last_active_at, +/-last_sign_in_at), email_address[], phone_number[], username[], web3wallet[], user_id[], query, created_at_after (Unix ms)
Returns: `{ data: User[], totalCount: number }`
```
...
### Metadata Merge and Replacement Semantics

`PATCH /v1/users/{user_id}/metadata` and `clerkClient.users.updateUserMetadata()` deep-merge the supplied `public_metadata`, `private_metadata`, and `unsafe_metadata` fields into existing metadata. Use `PUT /v1/users/{user_id}/metadata` only when the intent is to replace the user's metadata object.
